...
public class ProductController {
	...
	public Object findList(){
		...
		.map(item->{
			...
			return json;
		}).sorted((a,b) -> b.getInteger("price").compareTo(a.getInteger("price"))).findFirst()
		.map(item->"经过统计"+item.getString("name")+"商品销售量最高,销售了"+item.getInteger("count")+"件,销售额是:"+item.getInteger("price")).orElse("");
	...
	public Object addProduct(String productName,
		...
		po.setDeleteState(0);
		ProductPo product = productService.save(po);
		return product;
	}
	...
	public Object addProductCar(String productName) throws Exception {
	...
	public Object findProduct(String name) throws Exception {
